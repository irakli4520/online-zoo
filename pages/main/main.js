...
const sliderContainer = document.querySelector(
  ".animals-card .wrapper-card-item"
);
...
let reviewItemSize =
  reviewItem[0].clientWidth +
  +getComputedStyle(reviewItem[0]).marginRight.slice(0, 2) +
  5; //формирование ширины сдвига
if (windowWidth < 1221) {
  reviewItemSize =
    reviewItem[0].clientWidth +
    +getComputedStyle(reviewItem[0]).marginRight.slice(0, 2) +
...
function changeSlideInTime() {
  ...
  reviewCounter++;
  reviewWrapper.style.transform = `${translateConfig}(${
    -reviewItemSize * reviewCounter
  }px)`;
  clearInterval(startInterval);
  startInterval = setInterval(changeSlideInTime, changeTime);
  rangeInput.value = reviewCounter;
}
function pauseInterval(event) {
  //////////////////////////////////////////Создание паузы при нажатии на отзыв
...
function changeRangeInput() {
  ////////////////////////////////////////////Перелистывание слайдов по нажатию на инпут
  reviewWrapper.style.transition = "transform 0.4s ease-in-out";
  reviewCounter = +rangeInput.value;
  reviewWrapper.style.transform =
    "translateX(" + -reviewItemSize * reviewCounter + "px)";
  clearInterval(startInterval);
  startInterval = setInterval(changeSlideInTime, changeTime);
}
/*Навешивание событий*/
